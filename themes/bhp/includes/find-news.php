...
$con=mysqli_connect("localhost","root","mete4loW",'teambhp_site');

$sql_brand = mysqli_query($con, "select title, nid from node, field_data_field_make_filter where node.nid=field_data_field_make_filter.entity_id and node.type='make' and field_data_field_make_filter.field_make_filter_value='1' order by node.title") or die("Error in sql");
...
$sql_tag = @mysqli_query($con, "select title,nid from node  where type='news_category' ORDER BY node.title", 1) or die(mysqli_error());
...
<div class="roundAll5 marB10 findNews clearfix">
	...
	<div class="findNewsInner roundAll3">
		...
		<select id="newsBrandList"<?php if($node->type=='news') { ?> onchange="shownews_brand();"<?php } else if ($node->type=='page') { ?> onchange="shownews_bybrand(this.value);"<?php } ?>>
	...
	<div class="findNewsInner roundAll3 marT10">
		...
		<select id="newsTagList"<?php if($node->type=='news') { ?> onchange="shownews_tag()"<?php } else if ($node->type=='page') { ?> onchange="shownews_bytag(this.value);"<?php } ?>>
